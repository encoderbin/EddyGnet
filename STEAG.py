...
class Shortcut(nn.Module):
    def __init__(self, in_cha, out_cha):
        ...
        if in_cha != out_cha:
            self.conv = nn.Conv2d(in_cha, out_cha, 1, bias=False)
        else:
            self.conv = None
...
class AsymmetricConvolution(nn.Module):
    def __init__(self, in_cha, out_cha):
        ...
        self.shortcut = Shortcut(in_cha, out_cha)
...
class InteractionMask(nn.Module):
    ...
    def forward(self, dense_spatial_interaction, dense_temporal_interaction, threshold=0.5):
        ...
        spatial_interaction_mask = torch.where(spatial_interaction_mask > 0.95, spatial_interaction_mask,
                                               spatial_zero)

        temporal_interaction_mask = torch.where(temporal_interaction_mask > 0.8, temporal_interaction_mask,
                                               temporal_zero)
...
class SelfAttention_my(nn.Module):   # 位置坐标的属性已经被展为192维了，无法计算实际距离，放弃此思路，直接调低阈值吧

    def __init__(self, in_dims=2, d_model=64, num_heads=4):
        super(SelfAttention_my, self).__init__()
...
class TrajectoryModel(nn.Module):
    ...
    def __init__(self,
                 ...
                 obs_len=15, pred_len=7, n_tcn=5,
                 out_dims=192, num_heads=4):
        super(TrajectoryModel, self).__init__()

        self.number_gcn_layers = number_gcn_layers
        self.n_tcn = n_tcn
        self.dropout = dropout
        self.spa_in_dims=192 #--------------------
        self.tem_in_dims=193
        # sparse graph learning
        self.sparse_weighted_adjacency_matrices = SparseWeightedAdjacency(spa_in_dims=self.spa_in_dims, tem_in_dims=self.tem_in_dims, embedding_dims=embedding_dims, obs_len=obs_len, dropout=dropout,number_asymmetric_conv_layer=number_asymmetric_conv_layer)

        # graph convolution
        self.stsgcn = SparseGraphConvolution(
            in_dims=192, embedding_dims=embedding_dims // num_heads, dropout=dropout
        )

        self.fusion_ = nn.Conv2d(num_heads, num_heads, kernel_size=1, bias=False)
        self.SGCN_linear=nn.Linear(embedding_dims, self.spa_in_dims)
...
#         self.output = nn.Linear(embedding_dims // num_heads, out_dims)
    ...
    def forward(self, graph, identity):
        ...
        gcn_representation = self.fusion_(gcn_temporal_spatial_features) + gcn_spatial_temporal_features #N-head-T-16

        gcn_representation = gcn_representation.permute(2, 0, 1, 3)   ## T-N-head-F

        N,T,head,F_head=gcn_representation.shape
        gcn_representation=gcn_representation.reshape(N,T,(head*F_head)) ## T-N-embedding_dims
        SGCN_out=self.SGCN_linear(gcn_representation)  # T-N-256
        SGCN_out=SGCN_out.unsqueeze(0) # B-T-N-256
...
#         prediction = torch.mean(self.output(features), dim=-2)
        ...
        return SGCN_out.contiguous()*self.short
